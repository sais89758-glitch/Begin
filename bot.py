...
# Categories (ပြင်ဆင်ပြီးသား ၁၀ မျိုး)
CATEGORIES = [
    "1️⃣ အက်ရှင် (Action) 💥", 
    ...
    "9️⃣ နန်းတွင်းဇာတ်လမ်း 🏯", 
    "🔟 အိမ်ထောင်ရေးဇာတ်လမ်း 🏠"
]
...
async def admin_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🛠 **Admin Mode**\nဇာတ်ကားထည့်လိုသော Category ကို ရွေးပါ:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def receive_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ ရပါပြီ။\n\n🔗 **Episode 1 Link** ကို ပို့ပေးပါ။\n(နောက်ထပ်အပိုင်းများရှိလျှင် တစ်ခုချင်းစီ ဆက်တိုက်ပို့ပါ။ ပြီးရင် /done ကို နှိပ်ပါ)")
...
async def receive_episodes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ Episode {len(context.user_data['new_movie']['episodes'])} ရပြီ။\n\nထပ်ပို့ပါ (သို့မဟုတ်) ပြီးဆုံးရန် /done နှိပ်ပါ။")
    return SENDING_EPISODES
